...
class NilaiSikap extends Model
{
    ...
    public static function get_table_data($request)
    {
        $data = Siswa::select(DB::raw("
            ...
            siswa.id as `id`
        "))
        ->leftJoin(DB::raw('(select id_siswa, LEFT(sikap, 1) as `sikap` FROM nilai_sikap WHERE id_semester = '.Semester::get_active_semester()->id.') sq_nilai_sikap'), 'sq_nilai_sikap.id_siswa', '=', 'siswa.id')
