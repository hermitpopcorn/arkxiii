...
class NilaiAkhir extends Model
{
    ...
    public static function get_table_data($request)
    {
        $data = Siswa::select(DB::raw("
            ...
            CONCAT(siswa.id, ',', {$request->input('mapel')}) as `id`
        "))
        ->leftJoin(DB::raw('(select id_siswa, nilai_pengetahuan, nilai_keterampilan FROM nilai_akhir WHERE id_semester = '.Semester::get_active_semester()->id.' AND id_mapel = '.$request->input('mapel').') sq_nilai_akhir'), 'sq_nilai_akhir.id_siswa', '=', 'siswa.id')
