...
class Siswa extends MasterModel
{
    ...
    {
        $q = self::select(DB::raw("CONCAT(siswa.nis,'/',siswa.nisn) AS `NIS/NISN`, siswa.nama AS `Nama`, CONCAT(kelas.tingkat, ' ', jurusan.singkat, ' ', kelas.kelas, ' (', kelas.angkatan, ')') AS `Kelas`, siswa.nis as `foto`, siswa.id AS `id`"))->join('kelas', 'siswa.id_kelas', '=', 'kelas.id')->join('jurusan', 'kelas.id_jurusan', '=', 'jurusan.id');
        if($type == 'kelas') {
